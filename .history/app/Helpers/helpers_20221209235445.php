...
function getMediaColumnResource($mediaModel, $mediaCollectionName = '')
{
    ...
    if ($mediaModel->hasMedia($mediaCollectionName)) {
        return "<a href='" . $mediaModel->getFirstMediaUrl($mediaCollectionName) . "' data-lightbox='areajoint' title='" . $mediaModel->getFirstMedia($mediaCollectionName)->name . "'><img style='width:50px' src='" . $mediaModel->getFirstMediaUrl($mediaCollectionName) . "' alt='" . $mediaModel->getFirstMedia($mediaCollectionName)->name . "'></a>";
    }
}
