...
function getMediaColumnResource($mediaModel, $mediaCollectionName = '')
{
    ...
    if ($mediaModel->hasMedia($mediaCollectionName)) {
        return "<a href='" . $mediaModel->getFirstMediaUrl() . "' data-lightbox='OrdersPackPHPTest' title='" . $mediaModel->getFirstMedia()->name . "'><img style='width:50px' src='" . $mediaModel->getFirstMediaUrl($mediaCollectionName) . "' alt='" . $mediaModel->getFirstMedia($mediaCollectionName)->name . "'></a>";
    }
}
